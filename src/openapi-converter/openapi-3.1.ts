// NOTE: Type definitions includes only supported 3.1 OpenApi properties
...
export const primitiveTypes = [Type.Null, Type.String, Type.Number, Type.Boolean] as const
...
interface PrimitiveNode extends NodeBase {
    type: Type.Null | Type.String | Type.Number | Type.Boolean
}
...
interface MultyTypeNode extends NodeBase {
    types: string[]
}
...
export function treeToOpenApi(node: nodes.Node): Node {
    ...
    switch (node.type) {
        ...
        case nodes.Kind.Union:
            ...
            if (isMultiTypeSchema(variants)) {
                openApiNode = {
                    types: variants.map((n) => n.type),
                }
            } else {
                openApiNode = {
                    oneOf: variants,
        ...
        case nodes.Kind.Tuple:
            openApiNode = {
                type: Type.Array,
                prefixItems: node.items.map(treeToOpenApi),
...
function isMultiTypeSchema(variants: Node[]): variants is PrimitiveNode[] {
    ...
    for (let i = 0; i < variants.length - 1; i++) {
        const a: any = { ...variants[i] }
        delete a.type
        const b: any = { ...variants[i + 1] }
        delete b.type
